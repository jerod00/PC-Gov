...
**Phase 2:** Louisiana (done) — Missouri not yet built. See
...
| Houston | Under investigation (2026-07) — Beacon Bid's `robots.txt` is actually permissive (unlike Dallas/Fort Worth's Bonfire), but the open-solicitations page is a JS single-page app; the real listing loads via a client-side API call not visible in the raw HTML. If that endpoint turns out not to require auth (checking via browser DevTools), this one may be buildable after all | Use the portal's "Subscribe to Agency" feature in the meantime |
