...
| City of Dallas | Bonfire (`dallascityhall.bonfirehub.com`) `robots.txt` disallows all crawling; the city's own domain (`dallascityhall.com`) isn't blocked but has no live listing at all — its procurement page is static SharePoint content (policy FAQs, debrief-request info), confirmed via direct fetch. The only real bid data is on the blocked Bonfire subdomain. (**Dallas County** — a separate government entity — is a live source; see Phase 2 above, don't confuse the two.) | Register as a vendor on the Bonfire portal; enable commodity-code email notifications |
| Fort Worth | Bonfire (`fortworthtexas.bonfirehub.com`) `robots.txt` disallows all crawling; the city's own domain (`fortworthtexas.gov`, separate from Bonfire) returns "Access Denied" from Akamai's bot-mitigation layer even with a normal browser User-Agent — a different, tougher block than robots.txt, treated the same way (not something to route around) | Register as a vendor on the Bonfire portal; enable commodity-code email notifications |
...
| Waco | Its own domain (`waco-texas.com`) returns "Access Denied" from Akamai bot-mitigation on every request, same as Fort Worth's main site — its actual interactive platform is a Bonfire subdomain anyway, which would be blocked regardless | Register on the Bonfire portal for commodity-code email notifications |
