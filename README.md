...
src/
  sources/                # one module per source (sam_gov.py, tx_esbd.py, san_antonio.py, austin.py, oklahoma.py, louisiana.py, houston.py, dallas_county.py, wichita_falls.py, dallas_bonfire.py, fort_worth_bonfire.py, bonfire_rss.py, html_table.py, base.py)
...
> **Bonfire caveat (2026-07-20):** every Bonfire/Euna-family row below was marked blocked based on that specific portal's own `robots.txt`, checked directly — never inferred from another Bonfire portal's policy. That discipline turned out to matter: both City of Dallas's and City of Fort Worth's Bonfire subdomains were originally checked and found blocked, then re-checked much later and found *open*, each with a genuinely public per-organization "Opportunity RSS Feed" (see `src/sources/bonfire_rss.py`, `dallas_bonfire.py`, `fort_worth_bonfire.py`, all now live). Two Bonfire orgs' policy changing is still not evidence for any other — every remaining "confirmed blocked" row below still reflects its own last-checked date, not an assumption, and is worth periodically re-checking rather than treated as permanent.
