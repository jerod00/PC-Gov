...
src/
  sources/                # one module per source (sam_gov.py, tx_esbd.py, san_antonio.py, austin.py, oklahoma.py, louisiana.py, houston.py, dallas_county.py, wichita_falls.py, dallas_bonfire.py, html_table.py, base.py)
...
> **Bonfire caveat (2026-07-20):** every Bonfire/Euna-family row below was marked blocked based on that specific portal's own `robots.txt`, checked directly — never inferred from another Bonfire portal's policy. That discipline turned out to matter: City of Dallas's Bonfire subdomain was originally checked and found blocked, then re-checked much later and found *open*, with a genuinely public per-organization "Opportunity RSS Feed" (see `src/sources/dallas_bonfire.py`, now live). One Bonfire org's policy is not evidence for any other — every "confirmed blocked" row below still reflects its own last-checked date, not an assumption, and is worth periodically re-checking rather than treated as permanent.
...
| Fort Worth | Bonfire (`fortworthtexas.bonfirehub.com`) `robots.txt` was confirmed disallowing all crawling as of the original check — **not re-checked** since City of Dallas's policy turned out to have changed (see caveat above), so this should not be assumed still blocked without a fresh look; the city's own domain (`fortworthtexas.gov`, separate from Bonfire) separately returns "Access Denied" from Akamai's bot-mitigation layer regardless of robots.txt | Re-check `fortworthtexas.bonfirehub.com/robots.txt` and `.../opportunities/rss` directly; register as a vendor for commodity-code email notifications in the meantime |
...
| Harris County (Houston's county) | Also Bonfire, confirmed same blanket `robots.txt` block. Unlike Dallas County, Harris County's own domain (`purchasing.harriscountytx.gov`) has no standalone public listing to fall back on — its "Bids-Proposals" and "Online Solicitation Opportunities" pages are pure navigation/marketing content linking straight back to the blocked Bonfire portal, and a separate "Current Project Status" page redirects to a login wall | Register on the Bonfire portal for commodity-code email notifications |
...
| Waco | Its own domain (`waco-texas.com`) returns "Access Denied" from Akamai bot-mitigation on every request, same as Fort Worth's main site. Its actual interactive platform is a Bonfire subdomain — not individually re-checked against its own `robots.txt`/RSS feed (see Bonfire caveat above), so don't assume it's blocked without checking | Register on the Bonfire portal for commodity-code email notifications; check its Bonfire subdomain's `robots.txt`/`/opportunities/rss` directly before ruling it out |
...
| El Paso | Real bid data lives on `elpasotexas.ionwave.net` (Ion Wave, the same Euna Procurement family as Bonfire), confirmed same blanket `robots.txt` block as Tarrant County's Ion Wave portal. The city's own domain has no fallback — just links to Ion Wave or a "Closed & Awarded (Legacy System)" page (historical only) | Register on the Ion Wave portal for commodity-code email notifications |
| San Angelo | Also Bonfire (`cosatx.bonfirehub.com`), confirmed same blanket `robots.txt` block — the 7th Bonfire subdomain checked this session, the 7th to be blocked. The city's own domain has no fallback listing | Register on the Bonfire portal for commodity-code email notifications |
